...
RAW_FEATURE_COLUMNS = [  # 从 articles.csv 读取的原始类别字段
    ...
    "index_name",  # 索引名称
    "index_group_name",  # 索引组名称
]  # 结束原始特征列列表
ITEM_FILE_COLUMNS = [  # RecBole .item 文件输出字段（含类型后缀）
    "item_id:token",  # 商品 ID 列
    "product_type_name:token",  # 产品类型名称列
    "product_group_name:token",  # 产品组名称列
    "colour_group_name:token",  # 颜色组名称列
    "section_name:token",  # 区域名称列
    "garment_group_name:token",  # 服装组名称列
    "department_name:token",  # 部门名称列
    "index_name:token",  # 索引名称列
    "index_group_name:token",  # 索引组名称列
]  # 结束 item 文件列列表
...
def build_item_features(  # 构建 hm_seq.item 文件
    ...
    feature_df["item_id:token"] = feature_df["article_id"].map(canonical_item_id)  # 生成标准化 item_id
    ...
    for col in ITEM_FILE_COLUMNS[1:]:  # 清洗所有类别字段
        feature_df[col] = feature_df[col].map(clean_category_token)  # 执行字段清洗
