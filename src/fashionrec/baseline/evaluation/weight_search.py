...
from typing import Any, Literal  # 导入任意类型与模式注解
...
from fashionrec.baseline.ranking.fusion import ACTIVITY_WEIGHTS, ActivityTier  # 导入默认权重表与活跃度分层类型
from fashionrec.baseline.evaluation.offline_eval import (  # 导入离线评估相关组件
...
def run_weight_search(  # 运行完整权重搜索流程并保存结果
    ...
    compare_exclude_seen: bool | None = None,  # 兼容旧调用：True=both，False=false
    ...
    if compare_exclude_seen is not None:  # 兼容旧的布尔接口
        exclude_seen_mode = "both" if compare_exclude_seen else "false"
    modes_by_name = {"both": [False, True], "false": [False], "true": [True]}
    if exclude_seen_mode not in modes_by_name:
        raise ValueError(f"Unknown exclude_seen_mode: {exclude_seen_mode}")
    modes = modes_by_name[exclude_seen_mode]  # 确定要搜索的 exclude_seen 模式列表
...
def main(argv: list[str] | None = None) -> None:  # 命令行入口函数
    parser = argparse.ArgumentParser(prog="fashionrec weights", description="Search fusion weights on valid MAP@12")  # 创建参数解析器
    ...
    run_weight_search(  # 统一入口处理单模式或双模式搜索
