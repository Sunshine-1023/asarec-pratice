...
from fashionrec.recall.window_scores import filter_interactions_as_of  # as-of
...
def recall_repurchase(  # 复购召回
    ...
    *,
    ...
    history = {canonical_item_id(x) for x in user_history}  # 当前历史（可排除）
    scored: list[tuple[str, float]] = []  # 候选
    for item_id, (count, last_date) in stats.items():  # 每个买过 SKU
        if item_id in history:  # 已在当前 basket
            continue  # 跳过
