...
def main(  # 命令行入口函数
    ...
    *,  # 后续参数仅允许关键字传递
    default_config: str | Path = DEFAULT_CONFIG,  # 默认模型配置
) -> None:  # 无返回值
    parser = argparse.ArgumentParser(prog="fashionrec recall", description="Export SASRec/SASRecF top-k recall to CSV")  # 创建参数解析器
    parser.add_argument("--config", type=Path, default=Path(default_config))  # 模型配置文件路径
