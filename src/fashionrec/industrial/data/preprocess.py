...
from fashionrec.industrial.data.time import week_window_start
...
def _default_input_path() -> Path:  # 默认始终读取原始交易，禁止隐式复用旧 filtered 产物
    return RAW_PATH  # filtered 输入只能由当前数据准备流程显式传入
...
def load_transactions(  # 加载并预处理交易为 RecBole 列格式
    ...
    min_date = week_window_start(max_date, weeks)  # 计算窗口起始日期
