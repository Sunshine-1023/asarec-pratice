...
from fashionrec.shared.domain.ids import canonical_user_id
from fashionrec.shared.io import read_csv_rows, write_csv_rows
from fashionrec.industrial.recall.base import RecallChannel
...
def read_candidate_csv(path: str | Path) -> list[Candidate]:
    return [Candidate(**row) for row in read_csv_rows(path)]
