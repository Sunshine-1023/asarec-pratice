...
from fashionrec.industrial.data.split import (  # 时间切分与防泄漏路径约定
...
from fashionrec.industrial.ranking.fusion import (  # 导入融合相关函数
...
from fashionrec.industrial.recall.category_popular import (  # 类别热门召回
    CATEGORY_POPULAR_RECALL_TOP_K,  # 类别热门召回 Top-K 常量
    SEED_ITEMS as CATEGORY_SEED_ITEMS,  # 类别热门种子商品数常量
)  # 类别热门模块导入结束
from fashionrec.industrial.recall.item2item import (  # item2item 共现召回
    COOCCUR_WEEKS,  # 共现统计窗口（周）
    ITEM2ITEM_RECALL_TOP_K,  # item2item 召回 Top-K 常量
    SEED_ITEMS,  # 种子商品数常量
    TOP_SIM_K,  # 每个商品保留相似邻居数
)  # item2item 模块导入结束
from fashionrec.industrial.evaluation.experiment_report import (  # 排序器对照报告
...
from fashionrec.industrial.recall.popular import POPULAR_RECALL_TOP_K  # 导入热门召回 Top-K 常量
from fashionrec.industrial.recall.generator import generate_candidates, read_candidate_csv  # 与规则导出共享候选生成器
from fashionrec.industrial.recall.channel_registry import PrecomputedChannel, build_rule_channel_registry  # 规则注册表与序列适配器
from fashionrec.industrial.ranking.weighted_rrf import WeightedRRFRanker  # 排序层基线实现
...
def main(argv: list[str] | None = None) -> None:  # 命令行入口函数
    ...
    if args.weights_json is not None:  # 若指定了权重 JSON 文件
        from fashionrec.industrial.evaluation.weight_search import load_best_weights  # 导入权重加载函数
