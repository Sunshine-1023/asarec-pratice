...
def week_window_start(max_date: pd.Timestamp, weeks: int) -> pd.Timestamp:  # 计算含 max_date 的 N 周窗口起始日
    max_day = pd.Timestamp(max_date).normalize()  # 归一化到自然日
    return max_day - pd.Timedelta(days=weeks * 7 - 1)  # 含首尾共 weeks*7 天


def _week_window_start(max_date: pd.Timestamp, weeks: int) -> pd.Timestamp:  # 兼容旧内部名称
    return week_window_start(max_date, weeks)  # 转调公开函数
