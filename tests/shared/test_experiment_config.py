...
from fashionrec.shared.experiment.config import (  # 配置加载
    ExperimentConfigError,  # 配置错误
    classify_activity_tier,  # 分层
    load_experiment_config,  # 加载
)  # 导入结束
from fashionrec.industrial.ranking.fusion import classify_activity_tier as fusion_classify  # 现有融合分层，用于对齐
...
def test_baseline_experiment_config_loads() -> None:  # Baseline 正式 YAML 能加载且类型正确
    config = load_experiment_config("configs/baseline/experiment.yaml")
    assert config.experiment.name == "fashionrec_baseline"  # 应用独立实验名
