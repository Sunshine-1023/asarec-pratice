...
from src.data.manifest import build_processed_hm_manifest  # 数据清单
from src.evaluate.experiment_report import (  # 实验报告
    save_experiment_outputs,  # 落盘
    score_users,  # 计分
    utc_run_id,  # 运行 ID
)  # 报告模块结束
from src.evaluate.offline_eval import (  # 复用现有融合评估上下文
    FusionEvalContext,  # 上下文类型
    build_fusion_eval_context,  # 构建上下文
    default_sasrec_recall_csv,  # 默认序列召回路径
)  # 离线评估结束
from src.evaluate.weight_search import load_best_weights  # 只加载已冻结权重，不在此搜索
from src.experiment.config import ExperimentConfig, load_experiment_config  # 实验协议
from src.fusion.weighted_fusion import (  # 融合
...
def main() -> None:  # 命令行入口
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/experiment.yaml"))  # 实验配置
